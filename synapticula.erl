...
acquire(A) ->
  string:join(proplists:get_value(A,vesicle(),
  string:copies([[100,100]], 12)), [64]).
  ...
  case lists:member(A, keylist()) of
    true ->
      list_to_atom(lists:flatten(
        proplists:get_value(A, vesicle())));
    false ->
      list_to_atom(lists:flatten(
        lists:duplicate(12, [100,100])))
  end.

obtain(A) ->
  case lists:member(A, keylist()) of
    true -> string:tokens(acquire(A), [64,100]);
    false -> lists:duplicate(7, [100,100])
  end.
